...
def keep_longest_duplicate(df):
    # Calculate the length of each TU
    df['TU_length'] = abs(df['TU_start'] - df['TU_end'])
    
    df_sorted = df.sort_values(by=['Name', 'TU_length'], ascending=[True, False])
    df_deduplicated = df_sorted.drop_duplicates(subset=['Name'], keep='first')
    df_deduplicated['Name'] = df_deduplicated['Name'] 
    df_final = df_deduplicated.drop(columns=['TU_length'])
            ...
            for _, promoter_row in matching_promoters.iterrows():
                ...
                if direction == '+':
                    if promoter_row['Direction'] == direction and abs(promoter_pos - TU_start) < 500:
                        valid_promoters = 1
                        current_distance = abs(promoter_pos - TU_start)
                        if current_distance > furthest_distance:
                            furthest_distance = current_distance
                            TU_start = promoter_pos 
                            furthest_promoter_info = promoter_row          
                else:
                    if promoter_row['Direction'] == direction and abs(promoter_pos - TU_end) < 500:
                        valid_promoters = 1
                        current_distance = abs(promoter_pos - TU_end)
                        if current_distance > furthest_distance:
                            furthest_distance = current_distance
                            TU_end = promoter_pos  
                            furthest_promoter_info = promoter_row  
    
            if furthest_promoter_info is not None:
                TU_start_end = TU_start_end.append({'Name': f"{row['Transcription-Units']}",
                                        'Direction': direction,
                                        'TU_start': TU_start,
                                        'TU_end': TU_end,
...
TU_start_end = keep_longest_duplicate(TU_start_end)
#TU_start_end = rename_duplicates_with_numbering(TU_start_end)

print(TU_start_end.head())
TU_start_end.to_csv('/cluster/home/hugifl/TranscriptionalUnits/TU_start_end_added_promoters_V2.tsv', sep='\t', index=False)
